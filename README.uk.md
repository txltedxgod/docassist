...
- **Повністю асинхронно** — FastAPI + SQLAlchemy 2.0 (asyncpg); інгестія не блокує запити.
- **Справжній векторний пошук** — косинусна близькість у pgvector, контекст з обмеженням за токенами, інлайн-цитати з посиланнями на завантаження джерел.
- **Стрімінг** — відповіді надсилаються через Server-Sent Events; є й буферизований JSON-режим.
- **Відмовостійкість** — ретраї з експоненційною затримкою навколо LLM, єдиний формат помилок, структурне JSON-логування.
- **Два інтерфейси, один бекенд** — веб-дашборд і Telegram-бот на aiogram 3.
- **Зручно підтримувати** — міграції Alembic, типізація (mypy) та лінтинг (ruff), зелений CI на кожен push, юніт- та інтеграційні тести проти реального Postgres.
...
| `PUBLIC_BASE_URL` | `http://localhost:8000` | ��икористовується для посилань на завантаження джерел. |
